...
def _patch_qwen3_5_linear_attention_sequence_parallel() -> None:
    ...
    def sp_linear_forward(
        ...
        **kwargs,
        ...
        # Check if use_cache or other cache-related parameters are passed
        # Sequence parallel does not support decode with cached states
        use_cache = kwargs.get('use_cache', False)
        has_cache_params = cache_params is not None and getattr(cache_params, 'has_previous_state', False)

        # If use_cache is True or we have previous cache state, fall back to original forward
        if use_cache or has_cache_params:
            if not _sp_is_enabled(sequence_parallel_context):
                return origin_forward(
                    mod, hidden_states, cache_params=cache_params, attention_mask=attention_mask, **kwargs)
            # Even with SP enabled, fall back to non-SP forward for decode with cache
            # This is a workaround since sequence parallel does not support decode path
            return origin_forward(
                mod, hidden_states, cache_params=cache_params, attention_mask=attention_mask, **kwargs)
        ...
        if not _sp_is_enabled(sequence_parallel_context):
            ...
            return origin_forward(
                mod, hidden_states, cache_params=cache_params, attention_mask=attention_mask, **forward_kwargs)
