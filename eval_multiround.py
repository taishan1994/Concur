"""
使用 load_inline 生成 CUDA kernel 并在 kernelbench 上评估
支持多轮迭代优化
"""
import argparse
import json
...
NUM_THREADS = 8
NUM_GPUS = 8
OUTPUT_DIR = "data/kernelbench_multiround_results_qwen3.6-27B"
...
MAX_ITERATIONS = 4  # 最大迭代轮数

# ============================================================
# Template (same as generate_parallel_loadinline_kernelbench.py)
# ============================================================
...
# Multi-turn feedback prompt template (same as generate_parallel_loadinline_kernelbench.py)
MULTI_TURN_PROMPT = '''Now you have received the server feedback for your last implementation.
...
Return an improved cuda kernel implementation using load_inline.
'''
...
def process_one_sample_multiround(level, problem_name, pytorch_code, ref_arch_torch, ref_arch_kernel, output_dir):
    """Process a single KernelBench sample with multi-round iteration: API call + inline evaluation."""
    ...
    # ---- Step 1: Multi-round API Inference ----
    ...
    for iteration in range(MAX_ITERATIONS):
        print(f"[Thread] {label} - 迭代 {iteration + 1}/{MAX_ITERATIONS}")

        # Build prompt
        if iteration == 0:
            # First round: use full template with few-shot example
            user_content = (SYSTEM_PROMPT
                            .replace("$ref_arch_torch", ref_arch_torch)
                            .replace("$ref_arch_kernel", ref_arch_kernel)
                            .replace("$code", pytorch_code))
        else:
            # Subsequent rounds: use feedback from previous iteration
            prev_feedback = iteration_results[iteration - 1].get("feedback")
            if prev_feedback:
                feedback_str = json.dumps(prev_feedback, indent=2, ensure_ascii=False)
            else:
                feedback_str = "Feedback not available."
            user_content = MULTI_TURN_PROMPT.format(feedback=feedback_str)
        ...
        if feedback:
            ...
            correct = feedback.get("correctness", False)
            speedup = feedback.get("speedup", 0.0)
            if compiled and correct:
                print(f"[Thread] {label} - 迭代 {iteration + 1}: 完成 (speedup={speedup:.2f}x, correct=True)")
            else:
                error = feedback.get("error_message", "Unknown error")
                print(f"[Thread] {label} - 迭代 {iteration + 1}: 评估失败 - {error[:80]}")
        else:
            print(f"[Thread] {label} - 迭代 {iteration + 1}: 评估失败")
    ...
    # Use best result's feedback for final output
    best_feedback = iteration_results[best_iter_idx].get("feedback") if iteration_results else None
    ...
    result = {
        ...
        "messages": messages,
        "generated_code": iteration_results[best_iter_idx].get("generated_code", "") if iteration_results else "",
        "feedback": best_feedback,
        "speedup": max_speedup,
        "correctness": best_feedback.get("correctness", False) if best_feedback else False,
        "original_python_code": pytorch_code,
        "timestamp": time.time(),
        "num_rounds": len(iteration_results),
        "best_round": best_iter_idx + 1 if iteration_results else 0,
        "iterations": [
            {
                "iteration": it["iteration"],
                "compiled": it.get("feedback", {}).get("compiled", False),
                "correctness": it.get("feedback", {}).get("correctness", False),
                "speedup": it.get("feedback", {}).get("speedup", 0),
                "error": it.get("feedback", {}).get("error_message", ""),
            }
    ...
    status = f"best_speedup={max_speedup:.2f}x, correct={result['correctness']}, best_round={best_iter_idx + 1}"
...
def parse_args():
    p = argparse.ArgumentParser(description="Multi-round KernelBench evaluation with vLLM/SGLang API")
    ...
    p.add_argument("--max-iterations", type=int, default=None, help="Maximum number of iterations")
    return p.parse_args()


def main():
    global gpu_pool, API_PARAMS, NUM_THREADS, NUM_GPUS, OUTPUT_DIR, MAX_ITERATIONS
    ...
    print(f"Processing with {NUM_THREADS} threads, {NUM_GPUS} GPU(s), {MAX_ITERATIONS} iterations")
    ...
    # ---- Summary ----
    print(f"\n{'='*60}")
    print(f"Done. Processed {len(results)}/{len(tasks)} samples with {MAX_ITERATIONS} iterations each.")
    ...
    if results:
        ...
        print("By level (best result across all rounds):")
        for lv in KB_LEVELS:
            if lv in by_level:
                s = by_level[lv]
        ...
        for round_num in range(1, MAX_ITERATIONS + 1):
            round_results = [r for r in results if r.get("num_rounds", 0) >= round_num]
            round_correct = sum(1 for r in round_results if r.get("iterations", [{}])[round_num - 1].get("correctness", False))
            round_speedups = [r.get("iterations", [{}])[round_num - 1].get("speedup", 0) for r in round_results if r.get("iterations", [{}])[round_num - 1].get("speedup", 0) > 0]

            if round_results:
                print(f"  Round {round_num}: {round_correct}/{len(round_results)} correct ({round_correct/max(len(round_results),1)*100:.1f}%), avg speedup: {sum(round_speedups)/max(len(round_speedups),1):.2f}x" if round_speedups else f"  Round {round_num}: {round_correct}/{len(round_results)} correct")
